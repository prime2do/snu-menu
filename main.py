            ...
            for row in rows:
                ...
                if any(x in cafeteria_name for x in cafeteria_list):
                    ...
                    if raw_lunch and "등록된" not in raw_lunch:
                        # 메뉴 자르기(301: <TAKE-OUT>, 두레미담: <주문식 메뉴>, 3식당: <든든한끼샐러드코너>)
                        if "301" in cafeteria_name:
                            lunch_text = raw_lunch.split("<TAKE-OUT>")[0].strip()
                        elif "두레미담" in cafeteria_name:
                            lunch_text = raw_lunch.split("<주문식 메뉴>")[0].strip()
                        elif "3식당" in cafeteria_name:
                            lunch_text = raw_lunch.split("<든든한끼샐러드코너>")[0].strip()
                        else:
                            lunch_text = raw_lunch.strip()
